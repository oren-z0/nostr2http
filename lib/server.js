...
async function getResponse(
  requestMessage,
  {
    ...
    responseManipulatorModule, keys, nprofile,
  },
) {
  try {
    ...
    const newResponseMessage = await responseManipulatorModule({
      requestMessage, responseMessage, keys, destination, nprofile,
    });
...
exports.runServer = async function runServer(destination, options) {
  ...
  if (initalRelayUrls.length > 0) {
    ...
    nprofile = new Nip19Profile(keys.publicKey, goodRelays);
    ...
    const nprofileBech32 = nprofile.toBech32();
    console.info(
      `This entity includes both your public-key and hints for ${
        goodRelays.length
      } of the relays that your are connected to: ${nprofileBech32}`,
    )
    if (options.saveNprofileFile) {
      console.info("Saving nprofile-file");
      writeFile(options.saveNprofileFile, nprofileBech32);
    }
  } else {
